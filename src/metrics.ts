 ...
 *
 * Sanity gate: after session reopen, hydration anchors requestStartTime to the
 * preceding user message's created time. If that message is hours old (long
 * session) while the assistant message is recent, the raw difference would be
 * minutes — not a meaningful TTFT. Values above the threshold are reported as
 * null (rendered "--") rather than a misleading huge duration.
 */
