...
// SDK responses (hey-api with throwOnError=false) resolve to a fields-style
// result `{ data, error, response }`; some callers may hand us the bare array
// or object instead. Strip a single `{ data: ... }` wrapper when present.
function unwrapData(value: unknown): unknown {
  if (isRecord(value) && "data" in value) return value.data
  return value
}

function unwrapMessageList(response: unknown): readonly unknown[] {
  const value = unwrapData(response)
  return Array.isArray(value) ? value : []
}

// session.status() may resolve to a single status object or to a map keyed by
// sessionID; prefer the entry for the requested session when present.
function unwrapSessionStatus(response: unknown, sessionID: string): unknown {
  const value = unwrapData(response)
  if (isRecord(value)) {
    const bySessionID = value[sessionID]
    if (typeof bySessionID === "string" || isRecord(bySessionID)) return bySessionID
  }
  return value
}
...
async function observablePartTimes(api: HydrationApi, messageID: string, now: number): Promise<{
  readonly first: number | null
  readonly last: number | null
}> {
  const parts = api.client.message?.parts
...
export async function hydrateSession(input: HydrateSessionInput): Promise<boolean> {
  ...
  // TTFT anchor: the preceding user message's created time is the true
  // "request submitted" instant. In opencode 1.18.x, assistant.createdTime and
  // part.time.start are stamped in the same tick as the first text-start event,
  // so using assistant.createdTime here yields TTFT ≈ 0.
  const requestStartTime = toPerformanceTime(lastUser?.createdTime ?? assistant.createdTime, input.now) ?? input.now
  const completedTime = toPerformanceTime(assistant.completedTime, input.now)
  const sessionStatus = statusKind(unwrapSessionStatus(
  ...
  const partTimes = await observablePartTimes(input.api, assistant.messageID, input.now)
  ...
  // now a live event handler (e.g. session.next.step.started -> currentRequest)
  // may have replaced the requests entry with a newer in-flight request.
  // Overwriting it with this (stale) snapshot would orphan the live request
  // and misroute subsequent deltas. Re-check immediately before writing.
  const liveEntry = input.state.requests.get(input.sessionID)
  if (
    liveEntry !== undefined &&
    liveEntry !== current &&
    !liveEntry.isComplete &&
    liveEntry.messageID !== current.messageID
  ) {
    return false
  }
